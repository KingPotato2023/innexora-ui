// A calming nature photo with a daily-rotating inspirational quote overlaid.
// Image and quote both rotate based on day-of-year — same all day, fresh
// the next morning.

import { dailyQuote } from "../lib/daily-quote";

const ZEN_IMAGES: { url: string; alt: string }[] = [
  {
...
export function ZenStrip({ variant = "strip" }: { variant?: "strip" | "card" }) {
  ...
  return (
    <div
      className={
        "relative w-full overflow-hidden rounded-xl border border-ink/10 shadow-paper " +
        (isCard
          ? "h-full min-h-[320px]"
          : "h-[200px] sm:h-[240px]")
      }
    >
      ...
      <div
        className={
          "absolute inset-x-0 bottom-0 px-5 pb-5 text-white " +
          (isCard ? "sm:px-6 sm:pb-6" : "sm:px-7 sm:pb-6")
        }
      >
        <blockquote
          className={
            "italic font-medium leading-snug drop-shadow-sm " +
            (isCard
